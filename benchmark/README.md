...
```bash
# Run worker1_primary failover test on benchmark-1 directory
./worker_failover_benchmark1.sh

# Run worker2_primary failover test on benchmark-2 with custom parameters
./worker_failover_benchmark2.sh --failure-type worker2_primary -t read -u 200 -s 20 -r 180

# Run worker3_primary failover test on benchmark-3 with custom delay and duration
./worker_failover_benchmark3.sh --failure-type worker3_primary --delay 20 --duration 40
```
...
./worker_failover_benchmark.sh --dir benchmark-1 --failure-type worker1_primary -t mixed -r 180
```
...
# Run worker failover test on benchmark-2 with worker2_primary node
./run_benchmark_scenario.sh --scenario failover --dirs benchmark-2 --worker worker2_primary
