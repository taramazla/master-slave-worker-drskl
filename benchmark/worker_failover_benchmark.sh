...
FAILURE_TYPE="worker1_primary" # Options: worker1_primary, worker2_primary, worker3_primary
...
print_usage() {
    ...
    echo "                          Options: worker1_primary, worker2_primary, worker3_primary"
    ...
    echo "  $0 --dir benchmark-1 --failure-type worker1_primary --delay 15 --duration 30"
    echo "  $0 --dir benchmark-2 --failure-type worker2_primary --type read --runtime 180"
}
...
VALID_FAILURE_TYPES=("worker1_primary" "worker2_primary" "worker3_primary")
VALID_TYPE=false
...
get_container_name() {
    ...
    case $failure_type in
        worker1_primary)
            echo "citus_worker1_primary"
            ;;
        worker2_primary)
            echo "citus_worker2_primary"
            ;;
        worker3_primary)
            echo "citus_worker3_primary"
            ;;
        *)
